package com.fawnyr.travelplanbackend.manager;
...
//            multipartFile.transferTo(file);
     ...
     * @param filrUrl
     */
    public void validPicture(String filrUrl){
        // 校验非空
        ThrowUtils.throwIf(StrUtil.isBlank(filrUrl), ErrorCode.PARAMS_ERROR, "文件不能为空");
        // 校验url格式
        try {
            new URL(filrUrl);
        ...
        // 校验url协议
        ThrowUtils.throwIf(!filrUrl.startsWith("http://") && !filrUrl.startsWith("https://"),
                ErrorCode.PARAMS_ERROR, "仅支持http或https协议");
        // 发生HEAD请求验证文件是否存在
        HttpResponse response = null;
        try {
            response = HttpUtil.createRequest(Method.HEAD, filrUrl).execute();
            // 未正常返回，无需执行其他判断
